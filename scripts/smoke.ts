...
async function main() {
  ...
  const questions = [
    ...
    "오늘 점심 뭐 먹지?",
    "강남구에서 최근 6개월 쇠퇴 위험이 가장 높은 상권은?", // 범위 밖 → 안내(회귀: 조용히 마포구로 대체 금지)
    "부산 상권의 최근 폐업 현황은?", // 범위 밖 → 안내
    "마포구 전체 유동인구 감소가 심한 상권의 위험도는?", // 오탐 회귀: '유동인구'가 '~구' 오인식 안 되어야 함
    "강동구 상권 활성화 관련 조례를 찾아줘", // 회귀: document_search 경로도 범위 밖 감지해야 함(내용 유사해도 오매칭 금지)
  ];
